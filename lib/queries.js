    ...
    knex.transaction(function(trx) {
        ...
        knex.select('g.id', 'g.score', 'a.assignment_name', 'm.module_code', 'm.id AS module_id', 'a.max_mark', 'a.weight', 'a.semester')
            ...
            .then(function(rows) {
                ...
                _.each(rows, function(row) { // calculates the percentage score of the mark
                    row.percentage = ((row.score * 100) / row.max_mark).toFixed(1);
                })
...
var addUserModule = function(req, res) {
    ...
    var current = req.body.current ? 1 : 0;
...
var calculateWeightedAverage = function(rows) {
    ...
    var sumWeight = _.reduce(rows, function(a, row) { return a + row.weight; }, 0);

    return _.reduce(rows, function(a, row) {

        var percentScore = (100.0 / row.max_mark) * row.score;
        var weightedWeight = row.weight / sumWeight;

        return a + (percentScore * weightedWeight);
